...
namespace StyleSynthesis {
	...
	class CurveLineSnapping {

		// snap curves to lines

	public:

		CurveLineSnapping(vector<vector<vec3>> &curves, vector<vector<vec3>> &lines);
		~CurveLineSnapping();

	public:

		bool process();
		bool output(vector<vector<vec3>> &snapLines);
