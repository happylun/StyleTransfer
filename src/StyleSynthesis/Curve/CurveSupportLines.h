...
namespace StyleSynthesis {
	...
	class CurveSupportLines {

	public:

		CurveSupportLines(TTriangleMesh &mesh);
		~CurveSupportLines();
	...
	private:
		...
		bool extractSupportEdges();
		bool chainSupportEdges();

		bool splitCurve(vector<vec3> &inCurve, vector<vector<vec3>> &outCurves);
	...
	private:
		...
		vector<vec2i> mSupportEdges;
		vector<vector<vec3>> mSupportLines;
	};
}