...
bool SamplePoissonDisk::runSampling(int numSamples, bool redoSampling) {
	...
	if (StyleSynthesisConfig::mSample_VisibilityChecking) {
	...
	while ((double)mSamples.size() < numSamples * StyleSynthesisConfig::mSample_MinimumSampleRate) {
		// while not enough samples, redo sampling

		if (!calculateCDF()) return false;

		mSampleRadius = sqrt(mTotalArea / (sqrt(3.0)*mDesiredNumSamples / 2)) * RELATIVE_RADIUS;
...
bool SamplePoissonDisk::runSampling(double sampleRadius) {
	...
	mDesiredNumSamples = (int)((mTotalArea * 2 / sqrt(3.0)) / cml::sqr(sampleRadius / RELATIVE_RADIUS));
...
#pragma omp parallel for
	...
	if (StyleSynthesisConfig::mSample_AddVirtualGround) {
...
bool SamplePoissonDisk::pruneInteriorFaces() {
	...
	if (numFaces > StyleSynthesisConfig::mSample_MaximumCheckedFaceCount ||
		StyleSynthesisConfig::mSample_MaximumCheckedFaceCount == 0) {
...
bool SamplePoissonDisk::generateSamples() {
	...
	for(sampleID=0; sampleID<mDesiredNumSamples;) {
		...
		int faceID = SampleUtil::samplingCDF(mTriangleAreaCDF);
		if (!mTriangleFlags[faceID]) {
		...
		if(valid) {
			...
			failCount++;
			if (failCount > StyleSynthesisConfig::mSample_MaximumFailedCount) break;
		}
	}
#ifdef DEBUG_OUTPUT
...
bool SamplePoissonDisk::checkVisibility(TPoissonSample &sample) {
	...
	if (!StyleSynthesisConfig::mSample_VisibilityChecking) return true; // skip visibility checking
