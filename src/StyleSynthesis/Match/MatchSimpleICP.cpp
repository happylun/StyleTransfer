...
bool MatchSimpleICP::runAnyShape(
	int iteration,
...
{
	// align any two shapes with any initial orientation
	// will intialize transformations using 4 horizontal rotation and OBB alignment
	...
	for (int transID = 0; transID <= 4; transID++) {
		...
		if (transID < 4) {
			xform.rotate(Eigen::AngleAxisd(0.5*M_PI*transID, Eigen::Vector3d::UnitY()));
		} else {
			Eigen::Matrix3d sourceCS, targetCS;
			if (!MatchSimpleICP::preorient(sourceP, sourceCS)) return false;
			if (!MatchSimpleICP::preorient(targetP, targetCS)) return false;
			xform = targetCS.transpose() * sourceCS;
		}
		...
		if (!MatchSimpleICP::error(targetP, sourceXP, currentError)) return false; // NOTE: swap source and target
...
bool MatchSimpleICP::runRegularShape(
	int iteration,
	Eigen::Matrix3Xd &sourceP,
	Eigen::Matrix3Xd &sourceN,
	Eigen::Matrix3Xd &targetP,
	...
	double &outError)
{
	...
	if (!findNearestNeighbors(tree, source, neighbors)) return false;
	if (!sliceMatrices(target, neighbors, matched)) return false;
	Eigen::Matrix3Xd diff = matched - source;
	outError = (diff.transpose() * diff).diagonal().maxCoeff();
